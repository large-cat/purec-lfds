...
void libtest_tests_list_aso_alignment( struct lfds_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds_misc_validity *dvs )
{
  ...
  *dvs = LFDS_MISC_VALIDITY_VALID;
  ...
  // TRD : struct lfds_list_aso_element
  if( LIBTEST_MISC_OFFSETOF(struct lfds_list_aso_element,next) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds_list_aso_element,value) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;



  // TRD : struct lfds_list_asu_state
  if( LIBTEST_MISC_OFFSETOF(struct lfds_list_aso_state,dummy_element) % LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds_list_aso_state,start) % LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;
