...
void libtest_tests_list_asu_new_after( struct lfds_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds_misc_validity *dvs )
{
  ...
  /* TRD : run one thread per logical processor
           ...
           each thread loops, calling lfds_list_asu_new_element_by_position( LFDS_LIST_ASU_POSITION_AFTER ),
  ...
  *dvs = LFDS_MISC_VALIDITY_VALID;
  ...
  per_thread_counters = libshared_memory_alloc_from_unknown_node( ms, sizeof(lfds_pal_uint_t) * number_logical_processors, LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
  ...
  lfds_list_asu_insert_at_position( &lasus, &first_element, NULL, LFDS_LIST_ASU_POSITION_START );
  ...
  */
  ...
  lfds_list_asu_query( &lasus, LFDS_LIST_ASU_QUERY_SINGLETHREADED_VALIDATE, &vi, dvs );
  ...
  /* TRD : we have a leading element, after which all inserts occurred
           ...
           this is why we're not using lfds_list_asu_get_start_and_then_next()
  */
  ...
  while( *dvs == LFDS_MISC_VALIDITY_VALID and lasue != NULL )
  {
    element = LFDS_LIST_ASU_GET_VALUE_FROM_ELEMENT( *lasue );
  ...
  lfds_list_asu_cleanup( &lasus, NULL );
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION new_after_thread( void *libtest_threadset_per_thread_state )
{
  ...
  {
    LFDS_LIST_ASU_SET_KEY_IN_ELEMENT( (tpts->element_array+loop)->lasue, tpts->element_array+loop );
    LFDS_LIST_ASU_SET_VALUE_IN_ELEMENT( (tpts->element_array+loop)->lasue, tpts->element_array+loop );
    lfds_list_asu_insert_at_position( tpts->lasus, &(tpts->element_array+loop)->lasue, tpts->first_element, LFDS_LIST_ASU_POSITION_AFTER );
  }
