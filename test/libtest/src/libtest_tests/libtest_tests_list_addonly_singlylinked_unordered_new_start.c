...
void libtest_tests_list_asu_new_start( struct lfds_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds_misc_validity *dvs )
{
  ...
  /* TRD : run one thread per logical processor
           ...
           each thread loops, calling lfds_list_asu_new_element_by_position( LFDS_LIST_ASU_POSITION_START )
  ...
  */
  ...
  while( *dvs == LFDS_MISC_VALIDITY_VALID and LFDS_LIST_ASU_GET_START_AND_THEN_NEXT(lasus, lasue) )
  {
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION new_start_thread( void *libtest_threadset_per_thread_state )
{
  ...
  for( loop = 0 ; loop < tpts->number_elements_per_thread ; loop++ )
  {
    ...
    lfds_list_asu_insert_at_position( tpts->lasus, &(tpts->element_array+loop)->lasue, NULL, LFDS_LIST_ASU_POSITION_START );
  }
