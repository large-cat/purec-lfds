...
int main( int argc, char **argv )
{
  ...
  if( rv == 1 )
  {
    ...
    if( arg_data != NULL )
    {
      ...
      iterations = (lfds_pal_uint_t) arg_data->integer.integer;
    }
    ...
    if( arg_data != NULL )
    {
      ...
      memory_in_megabytes = (lfds_pal_uint_t) arg_data->integer.integer;
    }
  ...
  if( determine_erg_flag == RAISED )
  {
    ...
    else
    {
      ...
      switch( der )
      {
        case LIBTEST_MISC_DETERMINE_ERG_RESULT_SUCCESS:
          printf( "  The smallest ERG size with successful results is %llu bytes, which\n"
                  ...
                  "  \n"
                  "  In the file 'lfds_porting_abstraction_layer_processor.h', in all\n"
                  "  the sections for ARM, please replace the existing the value of the\n"
                  "  #define LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES to %llu, like so;\n"
                  "  \n"
                  "  #define LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES %llu\n",
      ...
      printf( "\n"
              ...
              "  LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES in the file\n"
  ...
  if( run_flag == RAISED )
  {
    ...
    for( loop = 0 ; loop < (lfds_pal_uint_t) iterations ; loop++ )
    {
