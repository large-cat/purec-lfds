...
void libtest_tests_ringbuffer_writing( struct lfds_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds_misc_validity *dvs )
{
  ...
  *dvs = LFDS_MISC_VALIDITY_VALID;
  ...
  re_array = libshared_memory_alloc_largest_possible_array_from_unknown_node( ms, sizeof(struct test_element) + sizeof(struct lfds_ringbuffer_element), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES, &number_elements );
  ...
  for( loop = 1 ; loop < number_elements ; loop++ )
  {
    ...
    lfds_ringbuffer_write( &rs, NULL, &te_array[loop], NULL, NULL, NULL );
  }
  ...
  lfds_ringbuffer_query( &rs, LFDS_RINGBUFFER_QUERY_SINGLETHREADED_VALIDATE, &vi, (void *) local_dvs );
  ...
  while( *dvs == LFDS_MISC_VALIDITY_VALID and lfds_ringbuffer_read(&rs, NULL, (void **) &te) )
  {
...
static libshared_pal_thread_return_t LIBSHARED_PAL_THREAD_CALLING_CONVENTION thread_simple_writer( void *libtest_threadset_per_thread_state )
{
  ...
  lfds_ringbuffer_write( tpts->rs, NULL, &tpts->te, &overwrite_occurred_flag, NULL, (void **) &te );
  ...
  while( current_time < start_time + TEST_DURATION_IN_SECONDS )
  {
    ...
    lfds_ringbuffer_write( tpts->rs, NULL, te, &overwrite_occurred_flag, NULL, (void **) &te );
