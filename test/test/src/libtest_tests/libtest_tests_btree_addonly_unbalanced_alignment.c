...
void libtest_tests_btree_au_alignment( struct lfds_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds_misc_validity *dvs )
{
  ...
  *dvs = LFDS_MISC_VALIDITY_VALID;
  ...
  // TRD : struct lfds_btree_au_element
  if( LIBTEST_MISC_OFFSETOF(struct lfds_btree_au_element,up) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds_btree_au_element,left) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds_btree_au_element,right) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;

  if( LIBTEST_MISC_OFFSETOF(struct lfds_btree_au_element,value) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;



  // TRD : struct lfds_btree_au_state
  if( LIBTEST_MISC_OFFSETOF(struct lfds_btree_au_state,root) % LFDS_PAL_ALIGN_SINGLE_POINTER != 0 )
    *dvs = LFDS_MISC_VALIDITY_INVALID_TEST_DATA;
