...
void libtest_tests_list_aso_new_ordered( struct lfds_list_asu_state *list_of_logical_processors, struct libshared_memory_state *ms, enum lfds_misc_validity *dvs )
{
  ...
  /* TRD : run one thread per logical processor
           ...
           calling lfds_list_aso_insert_element_by_position( LFDS_LIST_ASO_POSITION_ORDERED )
  ...
  */
  ...
  if( *dvs == LFDS_MISC_VALIDITY_VALID )
  {
    ...
    while( *dvs == LFDS_MISC_VALIDITY_VALID and LFDS_LIST_ASO_GET_START_AND_THEN_NEXT(lasos, lasoe) )
    {
