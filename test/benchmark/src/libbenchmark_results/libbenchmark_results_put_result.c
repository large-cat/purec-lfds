...
void libbenchmark_results_put_result( struct libbenchmark_results_state *rs,
                                      ...
                                      lfds_pal_uint_t windows_logical_processor_group_number,
                                      lfds_pal_uint_t result )
{
  ...
  r = libshared_memory_alloc_from_most_free_space_node( rs->ms, sizeof(struct libbenchmark_result), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
  ...
  LFDS_BTREE_AU_SET_KEY_IN_ELEMENT( r->baue, r );
  
  lfds_btree_au_insert( &rs->results_tree, &r->baue, NULL );
