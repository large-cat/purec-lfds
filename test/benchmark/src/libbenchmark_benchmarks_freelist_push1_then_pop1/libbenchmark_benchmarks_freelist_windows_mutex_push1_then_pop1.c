                                                                    ...
                                                                    struct libbenchmark_threadset_state *tsets )
{
  ...
  switch( numa_mode )
  {
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_SMP:
      fs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_datastructure_freelist_windows_mutex_state), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ...
      fe = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_datastructure_freelist_windows_mutex_element) * number_logical_processors, LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ...
      while( LFDS_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_thread_states,lasue) )
      {
        ...
        ptbs = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_benchmark_freelist_windows_mutex_push1_pop1_per_thread_benchmark_state), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
    ...
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_NUMA:
      ...
      */
      ...
      fs = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct libbenchmark_datastructure_freelist_windows_mutex_state), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      libbenchmark_datastructure_freelist_windows_mutex_init( fs, NULL );
      ...
      while( LFDS_LIST_ASU_GET_START_AND_THEN_NEXT(tsets->list_of_per_numa_states,lasue) )
      {
        ...
        */
        ...
        fe = libshared_memory_alloc_from_specific_node( ms, pns->numa_node_id, sizeof(struct libbenchmark_datastructure_freelist_windows_mutex_element) * number_logical_processors_in_numa_node, LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      ...
      {
        pts = LFDS_LIST_ASU_GET_VALUE_FROM_ELEMENT( *lasue );
        ptbs = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct libbenchmark_benchmark_freelist_windows_mutex_push1_pop1_per_thread_benchmark_state), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
        pts->users_per_thread_state = ptbs;
      }
    break;
    ...
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_NUMA_BUT_NOT_USED:
      ...
      */
      ...
      lfds_list_aso_query( logical_processor_set, LFDS_LIST_ASO_QUERY_GET_POTENTIALLY_INACCURATE_COUNT, NULL, (void *) &number_logical_processors );
      fe = libshared_memory_alloc_from_specific_node( ms, largest_pns->numa_node_id, sizeof(struct libbenchmark_datastructure_freelist_windows_mutex_element) * number_logical_processors, LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      for( loop = 0 ; loop < number_logical_processors ; loop++ )
        libbenchmark_datastructure_freelist_windows_mutex_push( fs, &fe[loop] );
...
/****************************************************************************/
void libbenchmark_benchmark_freelist_windows_mutex_push1_pop1_cleanup( struct lfds_list_aso_state *logical_processor_set,
                                                                       enum libbenchmark_topology_numa_mode numa_mode,
