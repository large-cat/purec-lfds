...
void libbenchmark_topology_generate_numa_modes_list( struct libbenchmark_topology_state *ts, enum libbenchmark_topology_numa_mode numa_mode, struct libshared_memory_state *ms, struct lfds_list_asu_state *numa_modes_list )
{
  ...
  switch( numa_mode )
  {
    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_SMP:
      tnn = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_topology_numa_node), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      tnn->mode = LIBBENCHMARK_TOPOLOGY_NUMA_MODE_SMP;
      LFDS_LIST_ASU_SET_VALUE_IN_ELEMENT( tnn->lasue, tnn );
      lfds_list_asu_insert_at_start( numa_modes_list, &tnn->lasue );
    break;

    case LIBBENCHMARK_TOPOLOGY_NUMA_MODE_NUMA:
      libbenchmark_topology_query( ts, LIBBENCHMARK_TOPOLOGY_QUERY_GET_NUMBER_OF_NODE_TYPE, (void *) (lfds_pal_uint_t) LIBBENCHMARK_TOPOLOGY_NODE_TYPE_NUMA, &numa_node_count );
