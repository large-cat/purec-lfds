...
/***** private prototypes *****/
static void libbenchmark_topology_internal_generate_thread_set_one_lp_per_lowest_level_cache( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms, struct lfds_list_asu_state *lp_sets );
static void libbenchmark_topology_internal_generate_thread_set_one_to_all_lps_per_lowest_level_cache( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms, struct lfds_list_asu_state *lp_sets );
static void libbenchmark_topology_internal_generate_thread_set_all_lps_per_lowest_level_cache( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms, struct lfds_list_asu_state *lp_sets );
static void libbenchmark_topology_internal_generate_thread_set_all_lps( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms, struct lfds_list_asu_state *lp_sets );
...
void libbenchmark_topology_generate_deduplicated_logical_processor_sets( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms, struct lfds_list_asu_state *lp_sets )
{
  ...
  while( LFDS_LIST_ASU_GET_START_AND_THEN_NEXT(local_lp_sets, local_lasue) )
  {
    ...
    local_lps = LFDS_LIST_ASU_GET_VALUE_FROM_ELEMENT( *local_lasue );
  ...
  */
  ...
  while( LFDS_LIST_ASU_GET_START_AND_THEN_NEXT(local_lp_sets, local_lasue) )
  {
    ...
    while( cr != 0 and LFDS_LIST_ASU_GET_START_AND_THEN_NEXT(*lp_sets, lasue) )
    {
      lps = LFDS_LIST_ASU_GET_VALUE_FROM_ELEMENT( *lasue );
    ...
    if( cr != 0 )
    {
      new_lps = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_topology_logical_processor_set), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
      new_lps->logical_processors = local_lps->logical_processors;
      LFDS_LIST_ASU_SET_VALUE_IN_ELEMENT( new_lps->lasue, new_lps );
      lfds_list_asu_insert_at_end( lp_sets, &new_lps->lasue );
    }
  }

  lfds_list_asu_cleanup( &local_lp_sets, NULL );
    ...
    */
    ...
    while( lps_count < loop+1 and lfds_btree_au_get_by_absolute_position_and_then_by_relative_position(&ts->topology_tree, &be, LFDS_BTREE_AU_ABSOLUTE_POSITION_SMALLEST_IN_TREE, LFDS_BTREE_AU_RELATIVE_POSITION_NEXT_LARGER_ELEMENT_IN_ENTIRE_TREE) )
    {
      ...
      if( 0 == libbenchmark_topology_node_compare_node_types_function(node, node_llc) )
      {
        ...
        // TRD : now add LP
        new_tns = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_topology_node_state), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
        *new_tns = *node_lp;
        LFDS_LIST_ASO_SET_KEY_IN_ELEMENT( new_tns->lasoe, new_tns );
        LFDS_LIST_ASO_SET_VALUE_IN_ELEMENT( new_tns->lasoe, new_tns );
        lfds_list_aso_insert( &lps->logical_processors, &new_tns->lasoe, NULL );
        lps_count++;
      }
    }

    LFDS_LIST_ASU_SET_KEY_IN_ELEMENT( lps->lasue, lps );
    LFDS_LIST_ASU_SET_VALUE_IN_ELEMENT( lps->lasue, lps );
    lfds_list_asu_insert_at_end( lp_sets, &lps->lasue );
  }
...
static void libbenchmark_topology_internal_generate_thread_set_one_to_all_lps_per_lowest_level_cache( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms, struct lfds_list_asu_state *lp_sets )
{
  ...
  */
  ...
  /* TRD : count the number of LPs under the lowest level type
           ...
           since we're walking LFDS_ADDONLY_UNBALANCED_BTREE_WALK_FROM_LARGEST_TO_SMALLEST
  */
  be_lp = be_llc;
    ...
    while( lps_count < lowest_level_type_count*(loop+1) and lfds_btree_au_get_by_absolute_position_and_then_by_relative_position(&ts->topology_tree, &be, LFDS_BTREE_AU_ABSOLUTE_POSITION_SMALLEST_IN_TREE, LFDS_BTREE_AU_RELATIVE_POSITION_NEXT_LARGER_ELEMENT_IN_ENTIRE_TREE) )
    {
      ...
      if( 0 == libbenchmark_topology_node_compare_node_types_function(node, node_llc) )
      {
        ...
        while( lp_count < loop+1 and lfds_btree_au_get_by_relative_position(&be_lp, LFDS_BTREE_AU_RELATIVE_POSITION_NEXT_SMALLER_ELEMENT_IN_ENTIRE_TREE) )
        {
          ...
          if( node_lp->type == LIBBENCHMARK_TOPOLOGY_NODE_TYPE_LOGICAL_PROCESSOR )
          {
            new_lp = libshared_memory_alloc_from_most_free_space_node( ms, sizeof(struct libbenchmark_topology_node_state), LFDS_PAL_ATOMIC_ISOLATION_IN_BYTES );
            *new_lp = *node_lp;
            LFDS_LIST_ASO_SET_KEY_IN_ELEMENT( new_lp->lasoe, new_lp );
            LFDS_LIST_ASO_SET_VALUE_IN_ELEMENT( new_lp->lasoe, new_lp );
            lfds_list_aso_insert( &lps->logical_processors, &new_lp->lasoe, NULL );
            lp_count++;
            lps_count++;
          }
        }
