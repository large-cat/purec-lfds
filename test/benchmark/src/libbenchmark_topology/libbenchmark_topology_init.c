...
int libbenchmark_topology_init( struct libbenchmark_topology_state *ts, struct libshared_memory_state *ms )
{
  ...
  lfds_btree_au_init_valid_on_current_logical_core( &ts->topology_tree, libbenchmark_topology_node_compare_nodes_function, LFDS_BTREE_AU_EXISTING_KEY_FAIL, NULL );

  rv = libbenchmark_porting_abstraction_layer_populate_topology( ts, ms );

  lfds_btree_au_get_by_absolute_position( &ts->topology_tree, &baue, LFDS_BTREE_AU_ABSOLUTE_POSITION_LARGEST_IN_TREE );
  ...
  lfds_btree_au_init_valid_on_current_logical_core( &ts->lp_printing_offset_lookup_tree, libbenchmark_topology_compare_lp_printing_offsets_function, LFDS_BTREE_AU_EXISTING_KEY_FAIL, NULL );
