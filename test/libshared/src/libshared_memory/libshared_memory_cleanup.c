...
/****************************************************************************/
void libshared_memory_cleanup( struct libshared_memory_state *ms, void (*memory_cleanup_callback)(enum flag known_numa_node_flag, void *store, lfds_pal_uint_t size) )
{
  ...
  if( memory_cleanup_callback != NULL )
  {
    ...
    lasue = LFDS_LIST_ASU_GET_START( ms->list_of_allocations );

    while( lasue != NULL )
    {
      lasue_next = LFDS_LIST_ASU_GET_NEXT( *lasue );
  ...
  lfds_list_asu_cleanup( &ms->list_of_allocations, NULL );
